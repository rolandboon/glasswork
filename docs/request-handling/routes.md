...
Start every route with the same rhythm:

1. Define DTOs separately for body/query/params.
2. Keep handlers thin: validate → delegate to a service → return data.
3. Set `public` intentionally and document responses.
4. Add tags + summaries so OpenAPI stays readable.
...
- [OpenAPI Guide](/request-handling/openapi) - How routes generate OpenAPI specs
