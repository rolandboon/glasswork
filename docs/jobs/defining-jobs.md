...
If the payload does not match the schema, the job will fail with an `InvalidJobPayloadError`. This error is treated as **permanent** and will not trigger retries, sending the message directly to the Dead Letter Queue (DLQ) if configured.
