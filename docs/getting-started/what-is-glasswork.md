...
Learn more in [Architecture Philosophy](/architecture/philosophy).
...
| **Middleware** | Hono functions that intercept requests. Used for auth guards, logging, and response transformation. |
| **Config** | Type-safe application configuration created with `createConfig()` and validated with Valibot. |
| **List Query** | A query builder for creating filterable, sortable, paginated API endpoints with Prisma. |
| **Filter Schema** | A Valibot schema defining allowed filter fields and operators for List Query. |
| **Exception Tracker** | An interface for capturing errors (e.g., CloudWatch, Sentry, AppSignal integration). |
| **Request Context** | Per-request data (requestId, userId) stored via AsyncLocalStorage, accessible anywhere. |
