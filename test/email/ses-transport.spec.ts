...
describe('SESTransport', () => {
  ...
  describe('send', () => {
    ...
    it('should handle email headers as tags', async () => {
      mockSend.mockResolvedValue({
        MessageId: 'ses-message-id-tags',
      });
      ...
      expect(mockSend).toHaveBeenCalledTimes(1);
      const callArgs = mockSend.mock.calls[0][0];
      expect(callArgs.input.EmailTags).toBeDefined();
      expect(callArgs.input.EmailTags).toHaveLength(2);
    });
  });
