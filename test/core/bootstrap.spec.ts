...
describe('bootstrap', () => {
  ...
  it('should resolve async factory providers during bootstrap', async () => {
    ...
    const config = container.cradle.asyncConfig as { apiKey: string; timeout: number };
  ...
  it('should allow services to depend on async factory providers', async () => {
    ...
    const apiService = container.cradle.apiService as ApiService;
  ...
  it('should resolve multiple async factory providers in correct order', async () => {
    ...
    expect(container.cradle.configA).toEqual({ name: 'A' });
    expect(container.cradle.configB).toEqual({ name: 'B' });
  ...
  it('should handle mixed sync and async factory providers', async () => {
    ...
    expect(container.cradle.syncValue).toEqual({ type: 'sync' });
    expect(container.cradle.asyncValue).toEqual({ type: 'async' });
  });
  ...
  it('should handle async factory that depends on another async factory', async () => {
    ...
    expect(container.cradle.derivedConfig).toEqual({ endpoint: 'https://api.example.com/v1' });
  });
});
