...
/**
 * Thrown internally when a job has exhausted all retry attempts.
 * This triggers the onJobDeadLetter hook and indicates the job
 * should be moved to the dead letter queue.
 */
export class RetryExhaustedError extends Error {
  ...
  constructor(jobName: string, attemptNumber: number, maxAttempts: number, cause: Error) {
    super(`Job "${jobName}" exhausted ${maxAttempts} retries after ${attemptNumber} attempts`);
